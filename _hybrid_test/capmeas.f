...
C                3 all brittle
C    57  SYCAP   upper bound on the DUCTILE flow stress:
C                  0  none -- the unbounded SGE form
C                  1  this card's own JH-2 intact strength surface at
C                     the current pressure, floored at sigma_jc
C                 >1  that absolute stress, floored the same way
C                The SGE term goes as 1/sqrt(h) with no bound of its
C                own. On the shipped sandstone card at the deck's own
C                median chip thickness it reached 1980 MPa at
C                eps_p = 0.10 -- that card's HEL is 1982 MPa, in a
C                90 MPa rock. Two branches of one material cannot have
C                the ductile one stronger than the brittle one's
C                elastic limit. SDV18 reports the capped value.
C
C-----------------------------------------------------------------------
C  STATEV  (20; *Depvar, delete=12)
...
C
      ...
      integer itcut, idcf, ihmode, imode
      integer jInfoArray(*)
C
...
C-----------------------------------------------------------------------
      ...
      sycmod = z0
      if (nprops .ge. 57) sycmod = props(57)
C
...
C       Elastic predictor.
        pnew = pold - rkd*dvol
C
C       Upper bound on the ductile flow stress: this same material's JH-2
C       intact strength surface at the current pressure and strain rate.
C
C       The strain-gradient term goes as 1/sqrt(h) and has no bound of its
C       own, so on the deck's own median chip thickness the sandstone card
C       reached sy = 1980 MPa at eps_p = 0.10 -- its own HEL is 1982 MPa,
C       in a 90 MPa rock. Two branches of ONE material cannot have the
C       ductile one stronger than the brittle one's elastic limit.
C
C       No new constant is needed: the bound is built from PROPS(1..17),
C       which are already the JH-2 card. Applied unconditionally, because
C       there is no regime in which exceeding it is correct. SDV18 reports
C       the capped value, so a run in which the cap binds is visible.
        edsd = edot/edot0
        if (edsd .lt. edmin) edsd = edmin
        rfacd = z1 + rc*log(edsd)
        if (rfacd .lt. z0) rfacd = z0
        pstrd = pnew/phel
        pitd = pstrd + tstar
        if (pitd .lt. tiny) pitd = tiny
        if (sycmod .le. z0) then
          sycap = z0
        else if (sycmod .le. z1) then
          sycap = ra*pitd**rn*rfacd*sighel
          if (sycap .lt. tiny) sycap = tiny
        else
          sycap = sycmod
        endif
        do 70 i = 1, ncomp
...
C       and hand back an elastic answer for a plastic point.
     ...
     1             ed0jc, sgec, rlam, sycap, sy0, sjc0, fsg0, eta0)
        sy   = sy0
        sjc  = sjc0
        fsge = fsg0
        eta  = eta0
     ...
     1                 ed0jc, sgec, rlam, sycap, sy, sjc, fsge, eta)
     ...
     1                 cjc, ed0jc, sgec, rlam, sycap, sy2, s2, f2, e2)
...
C       regime survives under a grit and the free surface does not.
        ...
        if (dep .gt. z0) then
          ...
          if (trix .lt. -20.d0) trix = -20.d0
          edr = (dep/max(dt, tiny))/ed0jc
          if (edr .lt. z1) edr = z1
          epf = (dj1 + dj2*exp(dj3*trix))*(z1 + dj4*log(edr))
     1          *(z1 + dj5*thom)
C         Bound eps_f itself rather than the triaxiality that feeds it.
C         1.d6 is "will not fail in this increment or any plausible
C         number of them", and it keeps exp() away from overflow at the
C         widened compressive limit.
          if (epf .gt. 1.d6) epf = 1.d6
          if (epf .gt. tiny) then
            dnew = dold + dep/epf
          else
            dnew = z1
...
C       Hydrostatic tensile cutoff, the same one the brittle branch
C       applies: P >= -T*(1-D). Softened by the ductile damage so a
C       failing point loses its tensile capacity as it fails.
        tcutd = rt*(z1 - dnew)
        if (tcutd .lt. z0) tcutd = z0
        if (pnew .lt. -tcutd) pnew = -tcutd
C
        qlim2 = (z1 - dnew)*sy
...
C       variable 1. Available ONLY when field variable 1 is not being
C       used to carry the chip thickness, i.e. ONLY in mode 0.
C
C       The test was ".ne. 1", which excludes mode 1 and lets modes 2 and
C       3 through. That is a real defect, not a style point: PROPS(56)=3
C       on a field-carrying deck is the DOCUMENTED way to get the
C       pure-JH-2 baseline (see semgrit/hybrid.py h_source), and with the
C       old test it multiplied the intact strength by a chip thickness in
C       mm -- order 1e-4 -- collapsing the rock's strength by four orders
C       of magnitude, silently. Measured through the driver: sigis 0.0505
C       -> 5.40e-06 and q 0.465 -> 2.01e-04 MPa. The forced-brittle and
C       forced-ductile ablation runs are the whole point of shipping both
C       modes, so they have to be right.
        het = z1
        if (nfieldv .ge. 1 .and. ihmode .eq. 0) then
...
C
C     fthm^2 on the GND group: see the note in the caller. Without it the
C     Taylor/GND term is temperature-invariant and holds a finite flow
C     stress on molten material.
      arg = (fthm*fthm)*sgec*eta/(sjc*sjc)
      if (arg .lt. z0) arg = z0
      if (arg .gt. z0) then
      ...
      sy = sjc*fsge
C
C     Cap at the caller's bound -- the JH-2 intact surface of this same
C     material at this same pressure. sycap <= 0 disables it.
      if (sycap .gt. z0) then
        scap = sycap
        if (scap .lt. sjc) scap = sjc
        if (sy .gt. scap) then
          sy = scap
          if (sjc .gt. tiny) fsge = sy/sjc
        endif
      endif
      return
      end
