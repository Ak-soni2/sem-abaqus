...
rem
rem  The datacheck submits under its OWN job name, micro_6um_check. A datacheck
...
call abaqus job=micro_6um_check input=micro_6um.inp user=vumat_grind2.for double=both cpus=1 datacheck
if errorlevel 1 (
  echo.
  echo  DATACHECK FAILED -- read micro_6um_check.dat, the error is a keyword or the
