@echo off
cd /d "%~dp0"
rem  15 um pad -- SAG on HVOF WC-Co, after Ghosh et al. 2021.
rem
...
rem
rem  The datacheck submits under its OWN job name, micro_15um_check. A datacheck
...
call abaqus job=micro_15um_check input=micro_15um.inp user=vumat_grind2.for double=both cpus=1 datacheck
if errorlevel 1 (
  echo.
  echo  DATACHECK FAILED -- read micro_15um_check.dat, the error is a keyword or the
  echo  material card, not the physics. Nothing has been solved yet.
  exit /b 1
)
echo.
echo  datacheck passed. Starting the solve -- about 162 h on 8 cores.
echo.
call abaqus job=micro_15um input=micro_15um.inp user=vumat_grind2.for double=both cpus=8 interactive
