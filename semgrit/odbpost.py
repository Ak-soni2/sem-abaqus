"""Write the script that turns a finished .odb into grinding numbers.

The pipeline used to stop at "job submitted", which leaves the two questions that
actually matter unanswered: did anything cut, and is the result trustworthy. Both are
in the .odb and neither is obvious from the viewport.

The emitted script runs under Abaqus' own Python::

    abaqus python <name>_postprocess_odb.py <job>.odb [<name>_report.json]

``abaqus python`` uses ``odbAccess`` directly, so it needs no CAE licence token and no
GUI. The script is written for both Abaqus Python 2.7 (2023 and earlier) and 3.x
(2024+): no f-strings, no dict comprehension over ``.keys()`` views, print imported
from ``__future__``.

**Untested against a real .odb.** There is no Abaqus on the machine this was written
on. The output names it reads are exactly the ones the deck requests -- that much is
checked mechanically -- but the first real run is the first real test, so every lookup
reports what it found rather than failing silently.
"""

from __future__ import annotations

_SCRIPT = r'''"""Post-process a semgrit grinding .odb: forces, energies, material removed.
...
def write_odb_postprocess_script(path: str) -> str:
    """Write the .odb post-processing script. Returns the path."""
    with open(path, "w", encoding="ascii", newline="\n") as fh:
        fh.write(_SCRIPT)
    return path


def history_outputs_required() -> tuple:
    """The ODB history names the script reads.

    Exposed so a test can confirm the deck actually requests them: a post-processing
    script asking for output the .inp never wrote is the classic silent failure, and
    it only shows up hours later when the job is finished.
    """
    return ("RF1", "RF2", "RF3", "RM3", "ALLIE", "ALLKE", "ALLAE")
