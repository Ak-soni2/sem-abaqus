...
**Job name: `sagv2_30um`.** If the Abaqus console prints `Abaqus JOB sagv2_30um`, this is
the deck running. Anything else — `micro_30um` especially — is an older copy
...
| | |
|---|---|
| cut depth | **295 nm** — the paper's *measured* chip thickness for this pad |
| dc | 80 nm (measured, sections 4.2) |
| **chip / dc** | **3.69** |
| elements through the cut | 18.4 at 16.0 nm |
| passes over one track | 11 |
| estimate | ~96 h on 8 cores |
...
The packager prints the model mass. It should read **1.42563e-13** — workpiece
plus the grain. If it reads 9.28000e-14, the grain has no mass and the deck
is an old one.

Then watch KINETIC ENERGY. It should leave zero within the first few output
frames, once the grain closes its 14.8 nm standoff — about 5%
...
`postprocess_odb.py` runs automatically and writes `sagv2_30um_sdv13.csv` (brittle
fraction per frame per pass) and `sagv2_30um_summary.json`.

## Expected outcome

**Brittle regions present.** chip/dc = 3.69, the deepest cut of the three, and the paper sees *several traces of brittle fracture* alongside plastic flow. If this one stays entirely ductile through all its passes, the energy criterion accumulates too slowly — a finding about the criterion, not a bug in the deck.
