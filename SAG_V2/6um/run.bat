@echo off
cd /d "%~dp0"
rem  SAG_V2 -- 6 um pad, WC-Co, after Ghosh et al. 2021.
rem
rem  JOB NAME: sagv2_6um
rem  If the console prints "Abaqus JOB sagv2_6um" you are running THIS deck.
rem  Anything else -- micro_6um in particular -- is an older copy.
rem
rem  Stages, in order:
...
call abaqus job=sagv2_6um_check input=sagv2_6um.inp user=vumat_grind2.for double=both cpus=1 datacheck
if errorlevel 1 (
  echo.
  echo  DATACHECK FAILED -- read sagv2_6um_check.dat. Nothing has been solved.
  exit /b 1
)
echo.
echo  datacheck passed. Solving sagv2_6um -- about 392 h on 8 cores.
echo.
call abaqus job=sagv2_6um input=sagv2_6um.inp user=vumat_grind2.for double=both cpus=8 interactive
