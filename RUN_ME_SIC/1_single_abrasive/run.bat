...
call abaqus job=single_abrasive_check input=single_abrasive.inp user=vumat_grind.for double=both cpus=1 datacheck
if errorlevel 1 exit /b 1
call abaqus job=single_abrasive input=single_abrasive.inp user=vumat_grind.for double=both cpus=8 interactive
