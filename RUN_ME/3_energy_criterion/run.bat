...
call abaqus job=energy_criterion_check input=energy_criterion.inp user=vumat_grind2.for double=both cpus=1 datacheck
if errorlevel 1 exit /b 1
call abaqus job=energy_criterion input=energy_criterion.inp user=vumat_grind2.for double=both cpus=8 interactive
