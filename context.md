...
> **The physics and mathematics are in [`THEORY.md`](THEORY.md)** — a
> self-contained derivation of every equation (d_c, h(u), JH-2, Johnson-Cook +
> strain gradients, the energy criterion, explicit stability), the provenance of
> each, and an explicit statement of what may and may not be quoted. This file
> covers code layout and status; that one covers the theory.
...
### Notebooks
- `SEM_TO_ABAQUS_GRINDING_WHEEL.ipynb` (37 cells), `SEM_TO_ABAQUS_MULTI_ABRASIVE.ipynb` (11 cells) — **current** deliverables. Both now render the full per-stage analysis (calibration -> segmentation -> measurement -> solids -> assembly -> chip field) from the user's own parameter choices, via `semgrit/figures.py`.
- `SEM_TO_ABAQUS_PRESENTATION.ipynb` (28 cells) — the presentation build, generated by `_make_presentation_notebook.py`, with a sample SEM image and result CSVs embedded so it runs standalone in Colab with no uploads. `--execute` runs it in place. Self-contained Colab notebooks with `semgrit`/`semgrit_multi` embedded as base64. Never hand-edited directly — always regenerated via `_make_notebook*.py`.
...
- **`CAE_SCRIPTS/`** — three hand-written Abaqus/CAE Python scripts (not pipeline-generated; only recently un-gitignored — it had been wrongly treated as generated output):
  ...
  - `CAE_SCRIPTS/verify_explicit_settings.py` — writes the model out to `.inp` and parses it back to confirm element types/section controls/step procedure actually took effect, rather than trusting the CAE GUI dialog state.

## 5. Deliverable packages (folder-by-folder)

> **The `.inp` files are not in git.** After the mesh refinement they are 88–155 MB each and eleven exceed GitHub's 100 MB limit, so `RUN_ME*/**/*.inp` (and the `.npy` field arrays) are gitignored. Rebuild with `python _make_run_packages.py --all` — deterministic, seeded, and gated. Everything else in those folders is tracked. `RUN_TRAJECTORY/**/*.inp` is gitignored on the same principle (63 MB across three decks; rebuild with `python _make_trajectory_jobs.py`), as is the CAD-harness scratch (`_cadshot_*.png`, `_cadcheck_*`, `_caddrive.*`).
...
- `RUN_TRAJECTORY/` — the three prescribed-trajectory SiC jobs (`ARC_26UM`, `ARC_80NM`, `RAMP_80NM`). Has its own `README.md`, `SUMMARY.json` and `trajectories.png`. Built by `_make_trajectory_jobs.py`.
- `semgrit/figures.py` — the 10 presentation figures (`calibration`, `segmentation_stages`, `segmentation_overlay`, `measurement_distributions`, `outline_fidelity`, `grain_gallery`, `solid_verification`, `assembly`, `chip_thickness`, `dc_forms`). Self-check: `python -m semgrit.figures`.
...
## 7. Project history / status timeline (Aug 2026)
...
3. **`1a126de`** (Aug 16, 2026) — "Track CAE_SCRIPTS: it was ignored as generated output, but is not." Housekeeping fix: the three hand-written `CAE_SCRIPTS/*.py` fixup/verification scripts were wrongly excluded by `.gitignore` (mistaken for pipeline-generated output); now tracked and documented.
4. **`9d50d2a`** (Aug 23, 2026) — "Single abrasive without the bond, a mesh that resolves dc, and figures." The single-abrasive decks lost the bond (only workpiece + abrasive, as intended); element aspect ratio was cut from 51:1; the mesh was refined until `dc` is spanned by `ELEMENTS_PER_DC = 5` elements; `run.bat`/`run.sh` corrected across all packages; and `REPOST/plots.py` added because no run of this project had ever produced a PNG. See `IMPROVEMENTS.md` for before/after numbers.
5. **`246722d`** (Aug 24, 2026, current HEAD) — "Show the work: per-stage figures, a driveable CAD viewer, three trajectory decks." Three strands:
   - **The pipeline now shows its work.** Feeding in an SEM image used to complete in two cards with nothing rendered, because the intermediates were discarded locals — a *data* problem, not a display problem. `segment_grains`/`measure_images` now optionally retain 14 named stages (`STAGE_KEYS`), a cache hit re-derives them (~0.6 s/image), and the new `semgrit/figures.py` (~1150 lines) draws 10 presentation figures from them. Wired into **both** main notebooks driven by the user's own parameters, plus a standalone `SEM_TO_ABAQUS_PRESENTATION.ipynb` with inputs embedded.
   - **Two latent bugs fixed in shared code.** (a) `_transition_station` bisected between block endpoints assuming `h(u)` was monotone; an arc crosses `dc` **twice** and is equally deep at both ends, so a deck that was 58 % brittle reported *"entirely ductile."* `transition_stations()` now scans for every root and `ChipField.transition_all_mm` carries them. (b) The regime test read the endpoints, which on an arc are both at zero depth, so it could only ever return ductile — it tests mid-span now. `verify_hybrid_deck.py` also learned the difference between a *derived* chip profile (check against the infeed) and a *prescribed* one (check card against report).
   - **`RUN_TRAJECTORY/`** — three prescribed-trajectory SiC jobs at `dc = 52.920 nm`: `ARC_26UM` (26 µm peak, 0.2 % ductile, 5.7 h), `ARC_80NM` (80 nm peak, 41.9 % ductile, two crossings at u = ±36.8 µm, 73.8 h) and `RAMP_80NM` (linear, 63.8 % ductile, 2.3 h). The supplied trajectory figure is ~650:1 vertically exaggerated, so the *shape* was reproduced exactly and the *length* derived from the real 25 mm wheel via `L = sqrt(8RD)`. Job 1 needs the grain library scaled ×7.716 because a 7 µm grit cannot cut 26 µm — `verify_rigid_deck` refuses it otherwise. See `RUN_TRAJECTORY/README.md` and §9.3 of `THEORY.md`.
   - The CAD viewer gained fullscreen, resize, collapsible sections, colour-by-property, explode, stencil-buffer section capping and 12 shortcuts — and three harnesses that did not exist before: `_cadviewer_check.py` (parses the JS with `node --check`), `_cadviewer_shot.py` (renders it), `_cadviewer_drive.py` (clicks every control in headless Chromium and fails on the first page error). The last one found a dead depth-of-cut slider and an `O` shortcut that threw.
   - **`THEORY.md` added** — the self-contained physics/mathematics document.
